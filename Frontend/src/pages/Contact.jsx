...
import { FaPhoneAlt, FaEnvelope, FaMapMarkerAlt, FaAward, FaUsers, FaHandshake } from "react-icons/fa";
import { Helmet } from "react-helmet-async";
import axios from "axios";
import { motion } from "framer-motion"; // <- animation library
import ContactBG from "../assets/images/design1.jpg"; // Full page background
  ...
  return (
    <div className="relative min-h-screen text-gray-100">
      ...
      <Helmet>
        ...
        <meta name="description" content="Get in touch with Ready Tech Solutions. Innovative IT solutions and services." />
      </Helmet>
      ...
      <div className="absolute inset-0 -z-10">
        <div
          ...
          style={{ backgroundImage: `url(${ContactBG})`, backgroundAttachment: "scroll" }}
        ></div>
        <div className="absolute inset-0 bg-black/70"></div>
      </div>

      {/* Hero Section */}
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="relative flex flex-col items-center justify-center h-[550px] text-center px-6">
        <h1 className="mb-4 text-5xl font-extrabold text-yellow-300 drop-shadow-md">Contact Ready Tech</h1>
        <p className="mb-10 text-lg text-gray-100/90">Collaborate with us to transform your ideas into cutting-edge technology.</p>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {[{ icon: <FaAward />, title: "6+ Years", desc: "Of Excellence" },
            ...
            <motion.div whileHover={{ scale: 1.05 }} key={i} className="p-6 transition shadow-lg bg-gray-800/70 rounded-xl">
      ...
      {/* Contact Info */}
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="max-w-6xl px-6 py-20 mx-auto">
        <h2 className="mb-8 text-3xl font-bold text-center text-yellow-300">Contact Information</h2>
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-3">
          {contactInfo.map((item, idx) => (
            <motion.div whileHover={{ scale: 1.05 }} key={idx} className="flex flex-col items-center p-6 text-center transition transform shadow-xl bg-gray-800/70 rounded-xl hover:shadow-2xl">
      ...
      {/* Contact Form + Map */}
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="max-w-6xl px-6 py-20 mx-auto">
        <h2 className="mb-8 text-3xl font-bold text-center text-yellow-300">Send Us a Message</h2>
        <div className="grid gap-12 sm:grid-cols-1 md:grid-cols-2">
          <motion.div whileHover={{ scale: 1.02 }} className="p-6 shadow-2xl bg-gray-800/70 rounded-xl md:p-8">
            <p className="mb-6 text-center text-gray-300">Our team will respond within 24 hours.</p>
            {contactResp && (
              <p className={`mb-4 font-semibold text-center ${contactResp.type === "success" ? "text-green-400" : "text-red-400"}`}>
                {contactResp.text}
              </p>
            )}
            <form className="space-y-4" onSubmit={handleContactSubmit}>
              ...
              <textarea name="message" placeholder="Your Message" rows="5" value={formData.message} onChange={handleChange} className="w-full px-4 py-3 text-gray-100 bg-gray-900 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-400 hover:shadow-md"/>
              <button type="submit" disabled={loadingContact} className="w-full py-3 font-medium text-white rounded-md bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-600 hover:shadow-xl">
                {loadingContact ? "Sending..." : "Send Message"}
              </button>
            </form>
          </motion.div>

          <motion.div whileHover={{ scale: 1.02 }} className="w-full overflow-hidden shadow-2xl h-72 md:h-96 rounded-xl">
            <iframe
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="max-w-6xl px-6 py-20 mx-auto">
        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-3">
          {locations.map((loc, idx) => (
            <motion.div whileHover={{ scale: 1.05 }} key={idx} className="p-6 text-center transition shadow-xl bg-gray-800/70 rounded-xl hover:shadow-2xl">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="max-w-6xl px-6 py-20 mx-auto">
        <h2 className="mb-8 text-3xl font-bold text-center text-yellow-300">Testimonials</h2>
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
          {testimonials.map((t, idx) => (
            <motion.div whileHover={{ scale: 1.02 }} key={idx} className="p-6 shadow-xl bg-gray-800/70 rounded-xl">
